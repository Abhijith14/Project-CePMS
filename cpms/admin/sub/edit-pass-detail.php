...
$eid=$_GET['editid'];
$sql="update tblpass set FullName=:fname,ContactNumber=:cnum,Email=:email,IdentityType=:itype,IdentityCardno=:icnum,Category=:cat,FromDate=:fdate,ToDate=:tdate where ID=:eid && CreatedBy=:createdby";
...
$eid=$_GET['editid'];
$createdby=$_SESSION['login'];
$sql="SELECT * from  tblpass where ID=$eid && CreatedBy='$createdby'";
$query = $dbh -> prepare($sql);
$query->execute();
